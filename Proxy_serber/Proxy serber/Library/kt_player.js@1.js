...
function kt_player(c_id, s, w, h, f, p) {
	...
	if (swfobject.hasFlashPlayerVersion('9.124.0')) {
		...
		swfobject.embedSWF(s, p_id, '100%', '100%', '9.124.0', 'expressInstall.swf', f, p, {'id': p_id, 'name': p_id});
...
function _kt_player_wrapper(c, p_id, f, s)
{
	...
	function configure_ad_on(ad, events) {
		...
		if (f[ad]) {
			...
			var iurl = '?mode=async&action=get_player_adv_html&adv_id=' + f[ad.substring(0, ad.length - 3)];
			if (s.indexOf('force_embed') >= 0)
			{
				iurl += '&force_embed=true';
			}
			iframe.src = iurl;
