...
if (s_adobe.c_r("omniture_optout") != 1 && s_URL.indexOf("/misc/optout") == -1) {
    ...
    if (s_URL.indexOf("adobe.com") != -1) {
        ...
        function s_adobe_genesis_dmdbase(a) {
            this.conf = a;
            this.tl = function() {
                if ("done" == this.c_r(this.conf.var_name)) return;
                if (typeof window[this.conf.var_name] == "object" && typeof window["s_gi"] != "undefined") {
                    var a = s_gi(window.s_adobe_account);
                    var b = window[this.conf.var_name];
                    var c = {},
                        d = this.conf.data_mapping;
                    if (b.demandbase_sid) {
                        if (d.sid) c[d.sid] = b.demandbase_sid;
                        if (d.company) c[d.company] = b.company_name;
                        if (d.industry) c[d.industry] = b.industry + "::" + b.sub_industry;
                        if (d.employee_range) c[d.employee_range] = b.employee_range;
                        if (d.revenue_range) c[d.revenue_range] = b.revenue_range;
                        this.slb(a, c, this.conf.link_name)
                    }
                    this.c_w(this.conf.var_name, "done", 0, this.conf.ck_domain, this.conf.ck_path)
                } else {
                    if (typeof this.conf.count == "undefined") this.conf.count = 1;
                    if (this.conf.count <= this.conf.check_max) {
                        this.conf.count = this.conf.count + 1;
                        setTimeout("s_adobe_genesis_dmdbase_i.tl()", this.conf.check_delay)
                    }
                }
            };
            this.r = function() {
                if ("done" == this.c_r(this.conf.var_name)) return;
                var a = window.location && window.location.href ? window.location.href : "";
                var b = a.indexOf("https") == 0 ? "https://" : "http://";
                var c = b + "api.demandbase.com/api/v2/ip.js?key=" + this.conf.key + "&var=" + this.conf.var_name + "&rnd=" + Math.floor(Math.random() * 10001);
            ...
            this.slb = function(a, b, c) {
                var d = "";
                for (var e in b) {
                    if (b[e]) a[e] = b[e];
                    d += (d ? "," : "") + e;
                    if (e == "events") {
                        a.linkTrackEvents = a.events = a[e]
                    }
                }
                if (d) {
                    a.linkTrackVars = d
                } else {
                    a.linkTrackVars = a.linkTrackEvents = "None"
                }
                a.tl(true, "o", c)
            };
            this.c_r = function(a) {
            ...
            this.c_w = function(a, b, c, d, e) {
                var f = new Date;
                f.setDate(f.getDate() + c);
                var g = escape(b) + (c == null || c == 0 ? "" : "; expires=" + f.toUTCString()) + "; path=" + e + "; domain=" + d;
                document.cookie = a + "=" + g
            }
        }
        s_adobe_genesis_dmdbase_i = new s_adobe_genesis_dmdbase({
...
try {
    if (typeof jQuery == "function") {
        if (location.pathname.toLowerCase().indexOf("/limited/canon.html") != -1 && typeof s_adobe == "object" ) {
