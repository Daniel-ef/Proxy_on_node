...
function load_prompt(evt, el, url) {
    $.get(url, function(data) {
        ...
        if (el.hasClass('copy')) {
            $.extend(doptions, { yes_text : 'Копировать', copy: true});
        }
    ...
    $('div.comment-form-container').each(function() {
        ...
        if ($form.length) {
            ...
            $cancel.click(function(event) {
                if (confirm("Вы потеряете все изменения в этом комментарии. Продолжить?")){
                    if (comment_in_form) {
...
function pickedTags(){
   ...
   var recolorItems = function(removed) {
        var tags = $.map($('div .tags.interesting').find("a").toArray(), function(tag) {
            return tag.innerHTML;
        });

        $(".short-summary").each(function() {
            var found = false;
            ...
            $(this).toggleClass("tagged-interesting", found);
        });
    };
...
$(document).ready( function(){
	...
	$("textarea").bind("keydown", new CUsersAutocomplete().onKeyDown)	
	$("textarea").bind("keypress", new CUsersAutocomplete().onKeyPressed)	
});

function yourWorkWillBeLost(e) {
    if(browserTester('chrome')) {
        return "Вы уверены, что хотите уйти? Ваша работа будет потеряна.";
    } else if(browserTester('safari')) {
        return "Вы уверены, что хотите уйти? Ваша работа будет потеряна.";
    } else {
        if(!e) e = window.event;
        e.cancelBubble = true;
        e.returnValue = 'Если Вы уйдете, Ваша работа будет утеряна.';
...
function LoadBanner(url, onsuccess) {
    try {
        if ($.browser.opera) {
            ...
            xhr.open( 'GET', "/api/proxy_request?url=" + encodeURIComponent(url));
            xhr.send();
        } else {
            $.ajax({
                crossDomain: true,
...
var adConstants = {
    refs: {
        webStorm: 'http://www.jetbrains.com/webstorm/?utm_source=hashcode&utm_medium=ws1_743x90&utm_campaign=webstorm',
        phpStorm: 'http://www.jetbrains.com/phpstorm/?utm_source=hashcode&utm_medium=ps1_743x90&utm_campaign=phpstorm',
        seoPult: 'http://hashcode.ru/special/seopult2/',
        softlineIntel: 'http://softline.ru/intei-parallel-studio?utm_source=hashcode&utm_medium=banner&utm_campaign=IPS2013(Q1)',
        sidebar: {
            webStorm: 'http://www.jetbrains.com/webstorm/?utm_source=hashcode&utm_medium=ws1_226x245&utm_campaign=webstorm',
            phpStorm: 'http://www.jetbrains.com/phpstorm/?utm_source=hashcode&utm_medium=ps1_226x245&utm_campaign=phpstorm'
        }
    },
    imgs: {
        question: {
            phpStorm: 'http://i.imgur.com/qpnrJyW.png',
            webStorm: 'http://i.imgur.com/IxdBnsd.png',
            seoPult: 'http://i.imgur.com/RLcgKf2.png',
            softlineIntel: 'http://i.imgur.com/LSEiaQd.png',
            softlineParallelStudio: "http://i.imgur.com/tjc7PeV.png",
            softlineClusterStudio: "http://i.imgur.com/9N6sWq0.png"
        },
        sidebar: {
            phpStorm: 'http://i.imgur.com/PmMEqPm.png',
            webStorm: 'http://i.imgur.com/mOMeSFe.png',
            softlineIntel: 'http://i.imgur.com/xUXX5Jb.gif',
            softlineParallelStudio: "http://i.imgur.com/p2DfRkA.png",
            softlineClusterStudio: "http://i.imgur.com/Q3mJxMq.png"
        }
    },
    titles: {
        softlineIntel: 'Разработчики любят Intel. Скачивай и обновляйся!',
        phpStorm: 'Лучшая IDE для PHP от JetBrains!',
        webStorm: 'Лучшая IDE для PHP от JetBrains!',
        seoPult: 'Автоматизированное продвижение сайтов'
    },
    owners: {
        jetBrains: 'JetBrains',
        softline: 'Softline',
    ...
    bannerTypes: {
        questionUpper: "body-top",
        questionLower: "body-lower",
        sidebarMiddle: "sidebar-middle"
    }
}
function SetUp(baner, src, ref, owner, title, yaCounter, tag){
    $("#banner-" + baner + "-img").attr('src', src);
    $("#banner-" + baner + "-img-a").attr('href', ref);
    $("#banner-" + baner).css('display', 'block');

    if (title) {
        $("#banner-" + baner + "-img-a").attr('title', title);
        $("#banner-" + baner + "-img").attr('alt', title);
    }

    _gaq.push(['_trackEvent', owner, baner + 'Viewed']);
    $("#banner-" + baner + "-img-a").click(function(){
        _gaq.push(['_trackEvent', owner, baner + 'Clicked']);
    });

    if (yaCounter){
        yaCounter.reachGoal(owner + 'Viewed');
        $("#banner-" + baner + "-img-a").click(function(event) {
            yaCounter4377841.reachGoal(owner + 'Clicked');
        });
    }
}

function localAd(onsuccess, onerror){
    var ip = $("#__ip__").text();
    if (!ip) return onerror();
    $.ajax({
        url: '/api/proxy_request?url=' + encodeURIComponent('http://api.wipmania.com/' + ip),
        success: onsuccess,
        error: onerror
    });
}

function CUsersAutocomplete() {
	var user_names;  // Name in lower case -> name

	CUsersAutocomplete.prototype.onKeyDown = function (e) {
		// e.target.completion_pos stores position of the current @
		// after which user name is added
		if (e.target.completion_pos != null && 
		      (e.keyCode == 38 || e.keyCode == 40)) {  // Up or down arrow
			var short = e.target.value.substring(e.target.completion_pos + 1, e.target.selectionStart).toLowerCase();
			var long  = e.target.value.substring(e.target.completion_pos + 1, e.target.selectionEnd);
			var extend_selection = 0;
			
			if (e.target.selectionStart == e.target.selectionEnd) {
				full_name = find_full_name(e.target.value, e.target.completion_pos + 1, e.target.selectionStart)
				if (full_name) {
					extend_selection = full_name.length - long.length;
					long = full_name;
				}
			}				
				
			var found_res = e.keyCode == 38 ? find_prev(short, long) : find_next(short, long);
			var cur_name_found = found_res[0]
			var found = found_res[1]
			//console.log([found, long, short]);
			
			if (found) {
				if (extend_selection > 0)
					e.target.selectionEnd += extend_selection;
				complete_name(e.target, short, found);
	    ...
	    if (e.target.completion_pos >= e.target.selectionEnd ||
		      e.target.completion_pos + 30 < e.target.selectionStart) 
			// User has left the zone near last @
			e.target.completion_pos = null;
    }	
	
	CUsersAutocomplete.prototype.onKeyPressed = function (e) {
		var user_names = CUsersAutocomplete.prototype.user_names;
		if (e.charCode == 64) {  // @
			if (typeof user_names == 'undefined') {
				build_users_list();
				user_names = CUsersAutocomplete.prototype.user_names;
			}
			
			e.target.completion_pos = e.target.selectionStart;
		}
		
 		if (e.target.completion_pos != null && 
			e.target.completion_pos < e.target.selectionStart) {
			var short = (e.target.value.substring(e.target.completion_pos + 1, e.target.selectionStart) + 
					String.fromCharCode(e.charCode)).toLowerCase();
 			var found = null;
			
			for (var i in user_names) {
				if (i.indexOf(short) == 0) {
					found = user_names[i];
					break;			
				}
			}
	...
	function complete_name(target, match_text, text) {
        target.value = target.value.substring(0, target.completion_pos + 1) +
			text + target.value.substring(target.selectionEnd);
        target.selectionStart = target.completion_pos + match_text.length + 1;
        target.selectionEnd   = target.completion_pos + text.length + 1;
        // console.log([target.value, target.completion_pos, target.selectionStart]);
    };

	function build_users_list() {		
		user_names = Array();
		
		var regexUsers = /<a\s[^>]*href\s*=\s*"(http:\/\/|)(hashcode\.ru|)\/users\/\d(?![^>]*\/(?:reputation|subscriptions)\/)[^>]*>([\s\S]*?)<\/a>/ig;
		var text = document.body.innerHTML;
		while ((match = regexUsers.exec(text)) != null) {
			var user_login = match[3].replace(/@| ♦+|<span[^>]*>([^<]+)<\/span>/ig, "$1");
        	var user_lower_case = user_login.toLowerCase();
			
			user_names[user_lower_case] = user_login
        }
		
	    CUsersAutocomplete.prototype.user_names = user_names;		
    }

	function find_full_name(text, name_start_pos, cur_name_end_pos) {
		var user_names = CUsersAutocomplete.prototype.user_names;
		
		for (var i = cur_name_end_pos + 1; i <= text.length; i++) {
			substr = text.substring(name_start_pos, i);
			if (substr.toLowerCase() in user_names)
				return substr;
		}			
		return null;
	}
	
	function find_prev(short, long) {
		var user_names = CUsersAutocomplete.prototype.user_names;
		var long_lower_case = long.toLowerCase();
		var prev_name, last_name;
		var cur_name_found = false;
		
		for (var i in user_names) {
			if (i == long_lower_case) {
				cur_name_found = true;
				if (prev_name)
					break;				
			} else	if (i.indexOf(short) == 0) { 
			    if (cur_name_found)
					last_name = user_names[i];
				else
					prev_name = user_names[i];
			}
		}		
		
		if (prev_name)
			return [cur_name_found, prev_name];
		else
			return [cur_name_found, last_name];
	}

	function find_next(short, long) {
		var user_names = CUsersAutocomplete.prototype.user_names;
		var long_lower_case = long.toLowerCase();
		var first_name, next_name;
		var cur_name_found = false;
		...
		if (next_name)
			return [cur_name_found, next_name];
		else
			return [cur_name_found, first_name];
	}
}